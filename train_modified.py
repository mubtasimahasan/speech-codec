...
def main(args):
    ...
    if args.continue_train and len(available_checkpoints) > 0:
    ...
    def distill_loss(feature, target_feature):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            loss_gen_all = mel_loss * 15.0 + loss_feature * 1.0 + loss_g * 1.0 + commitment_loss * 0.25 + codebook_loss * 1.0 \
                            + tot_f0_loss * 1.0 + tot_uv_loss * 1.0 + tot_content_loss * 5.0 + tot_spk_loss * 1.0 + 1.0 * loss_distill
                            #+ global_f0_loss * 1.0 + rev_global_f0_loss * 1.0 
            ...
            if iters % log_interval == 0 and accelerator.is_main_process:
                with torch.no_grad():
                    ...
                    if log_with_wandb is not None:
                        accelerator.log({"train/lr": cur_lr,
                                        "train/time": train_time_per_step,

                                        "grad_norm/encoder": grad_norm_g,
                                        "grad_norm/decoder": grad_norm_g2,
                                        "grad_norm/fa_quantizer": grad_norm_g3,
                                        "grad_norm/fa_predictors": grad_norm_g4,

                                        "train/loss_gen_all": loss_gen_all.item(),
                                        "train/loss_disc_all": loss_d.item(),
                                        "train/wav_loss": waveform_loss.item(),
                                        "train/mel_loss": mel_loss.item(),
                                        "train/distillation_loss": loss_distill.item(),
                                        "train/stft_loss": stft_loss.item(),
                                        "train/feat_loss": loss_feature.item(),

                                        "train/commit_loss": commitment_loss.item(),
                                        "train/codebook_loss": codebook_loss.item(),

                                        "pred/f0_loss": f0_loss.item(),
                                        "pred/uv_loss": uv_loss.item(),
                                        "pred/content_loss": content_loss.item(),
                                        "pred/spk_loss": spk_loss.item(),

                                        "rev_pred/rev_f0_loss": rev_f0_loss.item(),
                                        "rev_pred/rev_uv_loss": rev_uv_loss.item(),
                                        "rev_pred/rev_content_loss": rev_content_loss.item(),
                                        "rev_pred/x_spk_loss": x_spk_loss.item()
                                        }, step=iters)
                    else:
                        ...
                        writer.add_scalar("train/distillation_loss", loss_distill.item(), iters)
            ...
            if iters % (log_interval * 1000) == 0 and accelerator.is_main_process:
                with torch.no_grad():
                    ...
                    if log_with_wandb is not None:
                        accelerator.log({
                            'full/gt_audio': wandb.Audio(waves[0].cpu().numpy(), 
                            sample_rate=16000, 
                            caption="Full Ground Truth Audio")
                            }, step=iters)
                    else:
                    ...
                    if hasattr(model.quantizer, 'module'):
